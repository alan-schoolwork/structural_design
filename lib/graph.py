...
from lib.batched import batched
...
class graph(eqx.Module):
    ...
    @staticmethod
    def create() -> graph:
        return graph(_points=None)  # type: ignore

    def add_point(self, x: point) -> pointid:
        return _add_point(self, batched.one(x)).unwrap()

    def _add_points_raw(self, points: batched[point]) -> batched[pointid]:
...
@custom_vmap_
def _add_point(self: graph, x: batched[point]):
    return self._add_points_raw(x)


@_add_point.def_vmap
def _(
    axis_size: int,
    in_batched: tuple[graph, batched[point]],
    self: graph,
    x: batched[point],
) -> tuple[batched[pointid], batched[pointid]]:
    b1, b2 = in_batched
    assert not b1._points
    assert all(b2._bufs)

    out_batch = cast_unchecked()(
        tree_at_(lambda x: x._bufs, x, [True for _ in x._bufs])
    )
    return out_batch, _add_point(self, x)
...
# @dataclass
# class graph_builder:
#     _points: batched[point_builder, "0"]
